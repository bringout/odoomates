# Pre-init cleanup: drop the B2C tax-display group from any user that
# also has the B2B group, so the security/group.xml load (which mutates
# res.users.groups_id) doesn't trip account._check_one_user_type.
# Keeps the B2B group (tax-excluded) — accounting default for BiH.
import logging
...
def pre_init_hook(cr):
    ...
    _logger.info(
        "om_account_accountant pre_init_hook: dropping B2C tax group "
